...
实现了贴图函数   draw_bmp(const char* bmp_path, double x, double y, double width, double height) ;
实现了自定义按钮函数CreateImageButton(LPCSTR filePath, double xPosInch, double yPosInch, double widthInch, double heightInch, int buttonId)；
实现了按钮删除函数   DestroyButton(HWND buttonHandle); 
实现了交互文本框生成函数 CreateTextBox(double xPosInch, double yPosInch, double widthInch, double heightInch, int textBoxId)
实现了交互文本框删除函数 DestroyTextBox(HWND hwndTextBox);
内嵌按钮以及场景转化逻辑到 GraphicsEventProch函数 
*/
...
char wordle_array[5][5];  // 声明一个5x5的全局wordle字符数组
char wordle_current[5][5];//存储一个5x5全局显示字母数组
    ...
    {   
        case WM_COMMAND:
        {
            ...
            switch (wmId)
            {
                case 1:
                    DisplayClear();//销毁 清屏
					ButtonChoice=1;//可移动人物 
                    scene=2;//scene2为npc界面，scene6为迷宫界面 
                ...
                case 2:
                	...
                	DisplayClear();
                	if(inmenu==1||inmenu==2){//在菜单中的任一功能中
                		DestroyButton(buttonList[2]);
                		if(inmenu==2){
                ...
                case 3://start
					...
					if(scene==6){
					buttonList[80] = CreateImageButton("picture/play.bmp", 16, 1, 1, 1, 80); //绘制进入程序求解界面按钮
					DisplayClear();  
                    draw_bmp("picture/background.bmp",0,0,17,15);
                    draw_bmp("picture/rock.bmp",15.0,4.0,1,1);
                    draw_bmp("picture/rock4.bmp",16.0,4.0,0.75,1.5);
				...
				case 8://yes
					...
					DisplayClear();
					randomOrManualChoose(); //跳转界面3：随机生成or地图选择  
	                scene=51; //定义场景51        
	                 
					DestroyButton(buttonList[8]);//销毁8按钮 
	                break;
				case 9://hard
					DisplayClear();
					draw_bmp("picture/background.bmp",0,0,17,15);
                ...
                case 23://按下保存按钮 
                   ...
                   DisplayClear(); 
                   draw_scene_edit_maze(); 
                   draw_bmp("picture/success.bmp",15.5,5,1,1) ;//绘制一个显示成果保存的勾勾图案 
	               break;	
	            case 24://播放
	            	switch(musicmode){
						case 0:
                ...
                case 80:
                   ...
                   DestroyButton(buttonList[80]);
                   scene=9;//程序求解场景
                   buttonList[81] = CreateImageButton("picture/play.bmp", 16, 1, 1, 1, 81); //开始程序自动求解按钮 
                   buttonList[82] = CreateImageButton("picture/stop.bmp", 16, 2, 1, 1, 82); //暂停程序自动求解按钮 
                   buttonList[83] = CreateImageButton("picture/one.bmp", 16, 3, 1, 1, 83);  //单步执行按钮 
				   buttonList[84] = CreateImageButton("picture/exitbutton.bmp", 15, 0, 2, 1, 84); //返回按钮 
				   break; 
				case 81:
				   ButtonChoice=3;
				   break;
                ...
                case 60:
                    ...
                    randomNumber = rand() % 101; // rand() 生成一个随机数，% 101 确保这个数在0到100之间
                    wordflag=1;
                    }
...
void DestroyTextBox(HWND hwndTextBox) {
    if (hwndTextBox != NULL) {
        if (!DestroyWindow(hwndTextBox)) {
            ...
            hwndTextBox = NULL; // 窗口已被销毁，设置句柄为NULL
        }
    }
}
...
void fill_results_in_array(char array[5][5], const char* result, int count) {
    ...
    int row_to_fill = count; // 从底部开始填充，递减行索引
...
// T代表位置和字母都正确，Y代表字母正确但位置不对，R代表字母不在答案中
char* compare_words(const char* target, const char* guess) {
...
void print_array(char array[5][5]) {
    for (int i = 0; i < 5; i++) {
        for (int j = 0; j < 5; j++) {
            printf("%c ", array[i][j]);  // 打印当前元素并在元素间添加空格
        }
        printf("\n");  // 每完成一行的打印后换行
    }
}
